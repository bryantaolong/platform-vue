  ...
  <el-container>
    <div class="post-container">
      <div class="post-list">
        ...
        <p v-else-if="!pageData.content.length && !loading" class="no-data">
          您关注的用户暂未发布文章，或您还没有关注任何人。
        </p>
...
import { getFollowingPosts } from '@/api/post'; // Import the specific API
...
const formatNumber = (num: number) => {
  if (num === null || num === undefined) return '0'; // Handle null or undefined stats
  if (num >= 1000) {
...
const fetchFollowingPosts = async () => {
  loading.value = true;
  try {
    const response = await getFollowingPosts({ // Use getFollowingPosts API
    ...
    if (response.code === 200 && response.data) {
      ...
      // Handle API response not being 200, e.g., if not authenticated
      pageData.value.content = [];
      pageData.value.totalElements = 0;
      console.warn('Failed to fetch following posts:', response.message);
      // You might want to show a message to the user here, e.g., Element Plus ElMessage
    }
  } catch (error) {
...
// 初始化加载
onMounted(fetchFollowingPosts);
</script>

<style scoped>
/* You can reuse the existing styles from your main post list component */
/* For simplicity, I'm including them here. In a real project, you might */
/* consider extracting common styles into a shared CSS file or module. */
.post-container {
  width: 100%;
  max-width: 900px;
  margin: 0 auto;
